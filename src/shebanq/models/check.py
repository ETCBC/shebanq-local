...
class CHECK:
    ...
    def isInt(self, var, label, msgs):
        ...
        if type(val) in {list, tuple}:
            val = None if len(val) == 0 else val[-1]
    ...
    def isBool(self, var):
        ...
        if type(val) in {list, tuple}:
            val = False if len(val) == 0 else val[-1]
    ...
    def isId(self, var, tp, label, msgs, valrep=None):
        ...
        if type(valrep) in {list, tuple}:
            valrep = None if len(valrep) == 0 else valrep[-1]
