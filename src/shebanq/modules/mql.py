...
# sys.path.append("/usr/local/lib/emdros")
...
UGLY1_RE = re.compile(r"""\bhas\b[ \t\n]*['"]""", re.S | re.I)


def uglyCheck(query, msgs):
    """Prevent EMDROS from crashing.

    Some queries trip EMDROS up.
    It will crash without issuing a proper error message.
    Whenever users report this kind of behaviour, try to intercept queries with
    a suspicious pattern and prevent to feed it to EMDROS.

    This is what will be intercepted:

    *   Queries using the keyword "HAS" with a string operand.

    """
    if UGLY1_RE.search(query):
        msgs.append(("error", "You cannot use HAS with a string argument after it"))
        return False
